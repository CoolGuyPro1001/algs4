...
public:
     ...
     */
    template <typename T>
    static std::vector<T> indexSort(std::vector<T>& a)
    {
        int n = a.size();
...
private:
    ...
    ***************************************************************************/
    ...
    template <typename T>
    static bool isSorted(std::vector<T>& a, cmp_func)
    {
        return isSorted(a, 0, a.size(), cmp);
    }
    
    // is the array a[lo..hi) sorted
    template <typename T>
    static bool isSorted(std::vector<T>& a, int lo, int hi, cmp_func)
    {
        for (int i = lo + 1; i < hi; i++)
