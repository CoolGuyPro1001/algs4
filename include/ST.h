...
public:
    ...
    void put(Key key, Value val)
    {
        ...
        if (val == nullptr) st.erase(key);
        else             st.insert(std::pair<Key, Value>(key, val));
    }
    ...
    void remove(Key key)
    {
        if (key == nullptr) error("calls remove() with nullptr key");
        st.erase(key);
    }
    ...
    {
        if (key == nullptr) error("calls contains() with nullptr key");
    ...
    Key min() 
    {
        if (isEmpty()) error("calls min() with empty symbol table");
        return *st.begin();
    }
    ...
    Key max()
    {
        if (isEmpty()) error("calls max() with empty symbol table");
        return *st.lastKey();
    }
    ...
    Key ceiling(Key key)
    {
        ...
        Key k = st.lower_bound(key);
        if (k == nullptr) error("argument to ceiling() is too large");
        return k;
    }
    ...
    Key floor(Key key)
    {
        ...
        Key k = map_extended::floorKey(key);
        if (k == nullptr) error("argument to floor() is too small");
        return k;
    }
